...
n_classes = len(np.unique(y_train_points))
...
def build_model(
    ...
    input_points = keras.layers.Input(shape=input_shape)
...
test_loss, test_acc = model.evaluate(
    x_test_points, y_test_points, verbose=1)
...
# confusion matrix
predict_ans = np.argmax(model.predict(x_test_points),
                        axis=-1)  # *  argmax 找最大值的index
